...
function AdminPage() {
  ...
  // Load all admin metrics once we know we're admin
  useEffect(() => {
    if (!isAdmin) return;
    async function load() {
      try {
        const [profilesRes, cvsRes, dlRes, feedbackRes, referralsRes] =
          await Promise.all([
            supabase
              .from("profiles")
              .select("id, created_at, preferred_ui_language, role")
              .order("created_at"),
            supabase
              .from("cv_documents")
  ...
  const jobTypeData = toBarData(
    groupBy(data.cvDocs, (cv) => parseJobType(cv.title)),
  );

  const referralData = toBarData(
    groupBy(data.referrals, (r) => r.referral_code),
  );

  // Build a user_id → profiles map for feedback display
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-10">
        ...
        <section>
          <h2 className="text-base font-semibold text-gray-500 uppercase tracking-wide mb-4">
            Language breakdown (UI language)
          </h2>
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            {langData.length === 0 ? (
        ...
        {/* 5. Referral/partner performance */}
        <section>
          <h2 className="text-base font-semibold text-gray-500 uppercase tracking-wide mb-4">
            Partner referrals
          </h2>
                    ...
                    return (
                      <tr key={f.id} className="border-b border-gray-50 last:border-0 align-top">
                        ...
                        <td className="px-4 py-3 text-gray-700 max-w-xs">
                          {f.comment || <span className="text-gray-400">—</span>}
                        </td>
